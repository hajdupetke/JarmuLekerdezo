<x-app-layout>
  <div class="space-y-8 w-6/12 mx-auto mt-8">
    ...
    <form class="space-y-4" action={{route('incidents.store')}} method="POST" enctype="multipart/form-data">
      @csrf
      <div class="space-y-2">
        <label
      ...
      <div class="space-y-2 flex flex-col">
        ...
        <div class="flex items-center gap-2">
          <input type='checkbox' value={{$vehicle->license}} name='vehicles[]' id="vehicle-{{$vehicle->id}}" @checked(in_array($vehicle->license, old('vehicles') ?? []))>
      ...
      <button type="submit" class ="inline-flex my-3 items-center justify-center text-sm shadow-lg border border-gray-300 dark:border-gray-700 font-medium ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary h-10 px-4 py-2 rounded-full transition-all duration-500 hover:bg-gray-600  dark:bg-gray-800 dark:text-white cursor-pointer">Káresemény létrehozása</button>
